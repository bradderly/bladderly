...
class _SymptomSurveyModalState extends State<SymptomSurveyModal> {
  ...
  void onNext() {
    if (currentIndex < questions.length - 1) {
      ...
      asnwers.add(selectedOption!);
      context.read<SymptomSurveyBloc>().add(
            SymptomSurvey(
              userId: context.read<UserBloc>().state.userModelOrThrowException.id,
              score: Score(
                date: DateTime.now(),
                type: widget.symptom_type == 'IPSS' ? ScoreType.IPSS : ScoreType.OABSS,
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<SymptomSurveyBloc, SymptomSurveyState>(
      listener: (context, state) => switch (state) {
        SymptomSurveyInitial() => ProgressIndicatorModal.show(context),
        SymptomSurveySuccess() => {
            Navigator.of(context).pop(),
            ModalHelper.showModal(
              context: context,
              modalBuilder: (_) => SymptomResultModal(
