...
class PaymentBloc extends Bloc<PaymentEvent, PaymentState> {
  ...
  Future<void> _onRestorePlan(PaymentRestorePlan event, Emitter<PaymentState> emit) async {
    emit(const PaymentRestoreInProgress());

    final result = await _restorePlanUsecase();

    result.fold(
      (exception) => emit(PaymentRestoreFailure(exception: exception)),
      (plan) => emit(const PaymentRestoreSuccess()),
    );
  }
}
